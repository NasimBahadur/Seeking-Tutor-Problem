...
int main()
{
    ...
    for(int i = 0; i < total_student; i++) //create threads for student
    {
        ...
        pthread_create(&students[i], NULL, s_thread, (void*) &student_id[i]);
    }

    for(int i = 0; i < total_tutor; i++) //create threads for tutors
    {
        tutor_id[i] = i + 1;
        pthread_create(&tutors[i], NULL, t_thread, (void*) &tutor_id[i]);
    }

    pthread_create(&coordinator,NULL,c_thread,NULL); //create thread for coordinator
